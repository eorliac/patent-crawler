...
public class StatusUpdaterBolt extends AbstractStatusUpdaterBolt implements
	...
	if (!status.equals(Status.DISCOVERED)) {
	    System.out.println("#### StatusUpdaterBolt " + url + " " + status);
	}
        ...
        synchronized (waitAck) {
		    ...
		    System.out.println("##-- StatusUpdaterBolt " + url + " " + status + "! RETURN !");
	...
	if (!status.equals(Status.DISCOVERED)) {
	    System.out.println("#ES# StatusUpdaterBolt " + url + " " + status);
	}
	
        LOG.debug("Sent to ES buffer {} with ID {} and status {} --------", url, sha256hex, status);
    }
